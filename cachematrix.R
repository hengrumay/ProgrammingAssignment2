...
## The functions makeCacheMatrix() and cacheSolve() enable the inverse of an invertible matrix to be cached for retrieval without unnecessary recalculation.


## makeCacheMatrix: This function creates a special "matrix" object that can cache its inverse. Use with cacheSolve() function below.
...
# It first checks if the inverse has already been calculated (and the matrix has not changed). If the inverse exists, then the cacheSolve() would retrieve the inverse from the cache via getMatInv() function and skips the computation. Otherwise, it computes the inverse of the special "matrix" data and sets the value of the matrix-inverse in the cache via the setMatInv() function.
